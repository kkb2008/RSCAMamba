...
import timm
from geoseg.models import my_module
import sys
import os
...
class MambaLayer(nn.Module):
    def __init__(self, in_chs=1024, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16, channels=(64, 128, 256, 512)):
        super().__init__()

        self.my_module = my_module.KIntegration(in_chs, in_chs // 2, in_chs // 4)

        self.mamba = Mamba(
            d_model=in_chs // 2,  # Model dimension d_model
...
class Block(nn.Module):
    def __init__(self, in_chs=1024, dim=128, hidden_ch=256, out_ch=128, drop=0.1, d_state=16, d_conv=4, expand=2, last_feat_size=16):
        ...
        self.conv_ffn = ConvFFN(in_ch=in_chs // 2, hidden_ch=hidden_ch, out_ch=out_ch, drop=drop)
...
class Decoder(nn.Module):
    def __init__(self, encoder_channels=(128, 256, 512, 1024), decoder_channels=128, num_classes=6, last_feat_size=16):
        super().__init__()
...
class PyramidMamba(nn.Module):
    def __init__(self,
        ...
        self.backbone = timm.create_model(
            backbone_name, features_only=True, output_stride=32, img_size=img_size,
    ...
    def forward(self, x):
        ...
        res4 = res4.permute(0, 3, 1, 2)
        res5 = res5.permute(0, 3, 1, 2)
        data = [res2,res3,res4,res5]
        x = self.decoder(data)
