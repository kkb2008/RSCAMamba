...
class CityScapesDataset(Dataset):
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):
        ...
        img_ids = ['_'.join(id.split('_')[:4]) if id.split('_')[0] == 'repeat' else '_'.join(id.split('_')[:3]) for id in mask_filename_list]

        return img_ids

    def load_img_and_mask(self, index):
        img_id = self.img_ids[index]
        img_postfix = "_leftImg8bit"
        mask_postfix = "_gtFine_labelTrainIds"

        img_name = osp.join(self.data_root, self.img_dir, img_id + img_postfix + self.img_suffix)
        mask_name = osp.join(self.data_root, self.mask_dir, img_id + mask_postfix + self.mask_suffix)
