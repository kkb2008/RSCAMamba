...
from geoseg.datasets.woodscape_dataset import *
...
loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index), 
                 ...
                 FocalLoss(alpha=0.25, gamma=3, ignore_index=ignore_index,), 1.0, 1.75, 1.75)
...
def get_training_transform():
    train_transform = [
        albu.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.0),
...
def train_aug(img, mask):
    crop_aug = Compose([RandomScale(scale_list=[0.5, 0.75, 1.0, 1.25, 1.5], mode='range'),
...
def get_val_transform():
    val_transform = [
        albu.Lambda(
            image=ResizeToDivisor(divisor=32, padding_value=0), 
            mask=ResizeMaskToDivisor(divisor=32)
        ),
        albu.Normalize()
    ]
...
layerwise_params = {
    ...
    "backbone.patch_embed.norm": {"lr": 1e-6, "weight_decay": 0.0},
    "backbone.norm": {"lr": 1e-6, "weight_decay": 0.0},
    "absolute_pos_embed": {"lr": 1e-6, "weight_decay": 0.0},
    "relative_position_bias_table": {"lr": 1e-6, "weight_decay": 0.0},
...
layerwise_params.update({
    f'backbone.stages.{stage_id}.blocks.{block_id}.norm': {"lr": 1e-6, "weight_decay": 0.0}
    for stage_id, num_blocks in enumerate(depths)
    for block_id in range(num_blocks)
})
layerwise_params.update({
    f'backbone.stages.{stage_id}.downsample.norm': {"lr": 1e-6, "weight_decay": 0.0}
